...
def writeJson(lstURL, version, name, flag):
    lstWrite = []
    lstApp = []
    try:
        assert lstURL is not None, 'We need initiate a Permissions list'
...
def Service2():
    ...
    for i in elements:
        ...
        try:
            [urlsList, appName, appVersion, appPackage] = get_urls_apk(i)
            [PPurlsList, flagEmpty] = detect_urls_of_policy_privacy(urlsList)
            if flagEmpty == True:
                logger.info('We do not find URLs of Privacy Policy')
            else:
            ...
            if len(appList)> 0:
                ...
                writeJson(appList, appVersion, appName, flagEmpty)
                for url in appList:
                    print(url)
            if len(thirdList) > 0:
                logger.info('URLs that belong to third parties are :')
                writeJson(thirdList, appVersion, appName, flagEmpty)
                for url in thirdList:
                    print(url)
